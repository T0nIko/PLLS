...
def execute_file(file_name, mode):
    ...
    if mode == 'py':
        ...
        cc = "{4} {5} {0} {2}{3}{0}.{1} && {2}{3}{0}".format(file_name, mode, SRC_PATH, CPP_PATH, CPP_EXEC, CPP_FLAGS)
...
def create_file(data, task_id, mode, user_id):
...
def filechecker_wrapper(data, task_id, mode, uid):
    return check_res(execute_file(create_file(data, task_id, mode, uid), mode), task_id)
